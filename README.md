# Layered Prefill for vLLM Ascend

Owner-led migration carrier for optional layered-prefill scheduling and model execution. The extension must remain opt-in and distinguish the HUST integration from the MIT upstream project.

**Status: stage partitioning, request metadata, model-adapter declarations, and MoE cursor logic are installable and tested; vLLM Ascend attachment remains blocked until `HOST_CONTRACT.md` is implemented.**
...
Extension ID: `org.vllm-hust.layered-prefill`
...
vllm-hust-ext extension inspect org.vllm-hust.layered-prefill
vllm-hust-ext extension check org.vllm-hust.layered-prefill
pytest -q
```
